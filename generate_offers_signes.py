...
output_path = '/Users/christianvidalwolf/incremento sales/PriceAndQuantity_Ofertas_Signes.xlsm'

print("Loading sales data...")
df_sales = pd.read_excel(sales_path)

# Normalize SKU function for SG products
...
for row in data_rows:
    ...
    # Check if standard price is > 20 EUR
    if std_price <= 20.0:
        skipped_under_20 += 1
        continue
...
print(f"\nFiltered statistics:")
print(f"Skipped non-SG products: {skipped_non_sg}")
print(f"Skipped SG products with price <= 20 EUR: {skipped_under_20}")
print(f"Updated SG products with 0 sales (25% discount): {count_0_sales}")
print(f"Updated SG products with >0 sales (7% discount): {count_gt0_sales}")
print(f"Total updated SG products: {len(updated_rows)}")
